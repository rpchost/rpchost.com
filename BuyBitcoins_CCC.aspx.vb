...
Partial Class BuyBitcoins_CCC
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Request.QueryString("Type") = "1") Then
            lblTitle.Text = "DEPOSIT MONEY WITH CREDIT CARD"
            lblMsg.Text = "Once you make the payment you will be redirected to your Transactions page, our Gateway Payment partner will send a Passcode (One Time Password) to your phone, once you receive it, write it in your Transaction Passcode and click [Validate PassCode]. Please note that a fee (1 to 4%) is deducted by the gateway payment company depending on your bank location."
            type = "1"
        ElseIf (Request.QueryString("Type") = "2") Then
            lblTitle.Text = "DEPOSIT MONEY WITH PAYPAL"
        ...
        ElseIf (Request.QueryString("Type") = "3") Then
            lblTitle.Text = "DEPOSIT MONEY WITH BANK TRANSFER"
        ...
        ElseIf (Request.QueryString("Type") = "4") Then
            lblTitle.Text = "DEPOSIT MONEY WITH PERFECTMONEY"
        ...
        ElseIf (Request.QueryString("Type") = "5") Then
            lblTitle.Text = "DEPOSIT MONEY WITH WESTERUNION"
        ...
        ElseIf (Request.QueryString("Type") = "8") Then
            lblTitle.Text = "DEPOSIT MONEY WITH TRANSFERWISE"
            'lblMsg.Text = ""
            type = "8"
            'ddlCurrency.SelectedValue = "USD"
            'ddlCurrency.Enabled = False
        ...
        ElseIf (Request.QueryString("Type") = "9") Then
            ''lblTitle.Text = "DEPOSIT MONEY WITH CREDIT CARD"
            'lblMsg.Text = ""
            type = "9"

            Response.Redirect("Crd.aspx")
        ...
        If (PaymentStat <> "") Then
            ...
            If (PaymentStat = "Pending") Then
                lblVerification.Text = "You have a transaction in <a href='transactions.aspx' class=RedTextBold title='Transactions history'>PENDING</a> mode. [Transaction ID = " & TransID & "]"
                btnCancelPayment.Enabled = True

            ElseIf (PaymentStat = "Process") Then
                lblVerification.Text = "You have a transaction in <a href='transactions.aspx' class=RedTextBold title='Transactions history'>PROCESS</a> mode. [Transaction ID = " & TransID & "]"
    ...
    Protected Function isUserHasPendingPayments(ByVal userId As Integer, ByVal type As String) As String
        ...
        End If
        If (type = "9") Then
            reader = opExec.rdGetReader("select * from Payments where (Status='Pending' or Status='Process') and UserID = " & Session("UserID") & " and OrderType='Buy' and paymentMethod = 'CARD' ")
        End If
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        Else
            ...
            If (Request.QueryString("Type") = "1") Then
                ...
                Response.Redirect("CrCrdd.aspx")
            End If
            ' Response.Redirect("CrCd.aspx")
            If (Request.QueryString("Type") = "2") Then Response.Redirect("Paypall.aspx")
            If (Request.QueryString("Type") = "3") Then Response.Redirect("BankTransfers.aspx")
            ...
            If (Request.QueryString("Type") = "9") Then Response.Redirect("Crd.aspx")
        ...
        If (isUserHasPendingPayments(Session("UserID"), type).Trim.Equals("Pending")) Then
            ...
            If (Request.QueryString("Type") = "5") Then
                ...
                Try
                    ...
                    SendNotification.Email("BUY BTC Payment with WesterUnion canceled by you on: " & zone.GenerateTimeZone_Static1("Eastern Standard Time"), "Payment Details : " & vbCrLf & vbCrLf & " UserID = " & Session("UserID") & "<br><br>Best Regards<br>Rpchost.com ", Session("Email"))
                Catch ex As Exception
                    Response.Write(ex.Message)
                Finally
                    opExec.closeConnection()

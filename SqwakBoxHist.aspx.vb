...
#End Region
    ...
    Public dds As DataSet
    Public Shared boolGetData As Boolean
    ...
    Sub BindGrid()
        ...
        Dim da As New OleDbDataAdapter("select ID,dtDate,SqwakText,DateNow,statements,TextStatement from SqwakBox where year(dtDate) = year(now) and Month(dtDate)=Month(Now) and Day(dtDate) < Day(Now)  order by dtDate DESC", cnn)
        Dim ds As New DataSet
        da.Fill(ds, "EconCal")
        DataGrid1.DataSource = ds
        dds = ds
        'Response.Write("tt = " & dds.Tables("EconCal").Rows(0).Item(0))
        'Response.End()
        DataGrid1.DataBind()
    End Sub
    ...
    Private Sub DataGrid1_EditCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid1.EditCommand
        ...
        Response.Write("window.open('AntGestPop.aspx?PatientID=" & Session("PatientID") & "&AntID=" & e.Item.Cells(0).Text & " ','CustomPopUp','width=700, height=450, menubar=no, resizable=no,scroller=yes,left=200,top=150')")

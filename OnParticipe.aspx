<%@ Page Language="VB" AutoEventWireup="false" CodeFile="OnParticipe.aspx.vb" Inherits="OnParticipe" %>

<%@ register tagprefix="uc1" tagname="right" src="headers/rightheader.ascx"%>
<%@ register tagprefix="uc2" tagname="right" src="headers/upperheader.ascx"%>
<%@ register tagprefix="uc4" tagname="right" src="headers/headerbutton.ascx"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
	<head>
<link rel="icon" 
      type="image/png" 
      href="favicon.ico"/>
   
   <!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-131430617-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-131430617-1');
</script>


   		<title>Cryptocurrency trading | Buy and Sell Bitcoin | Online forex trading</title>
   		
		<META NAME="AREA" CONTENT="Forex Finance">
		<META HTTP-EQUIV="AUDIENCE" CONTENT="General">
		<META NAME="SUBJECT" CONTENT="Forex Signals, forecasts and analysis">
		<META NAME="PLACENAME" CONTENT="Forex Traders Heaven">
		<META NAME="CREATOR" CONTENT="Rpchost.com">
		<META HTTP-EQUIV="Content-language" CONTENT="en-US">
		<META NAME="COPYRIGHT" CONTENT="Rpchost.com">
		<META NAME="DESCRIPTION" CONTENT="The best cryptocurrency trading online, Buy and Sell your Bitcoin in a fast, secure and easy way">
		<META NAME="KEYWORDS" CONTENT="Buy, Cryptocurrency, Bitcoin">
		<META NAME="ABSTRACT" CONTENT="The best cryptocurrency trading online, Buy and Sell your Bitcoin in a fast, secure and easy way">
		<META name="robots" content="index, follow">
		<META NAME="AUTHOR" CONTENT="Rpchost.com">
    
<LINK rel="stylesheet" type="text/css" href="StyleSheet.css">

<meta name="verify-admitad" content="2504a66eeb" />	

  </head>
	<body leftMargin=0 topmargin=0 > 
	<form id="Form1" runat=server>
								
									<TABLE borderColor="#3366CC" height="100%" cellSpacing="0" cellPadding="0"  width="100%" align="center"
										border="0">
																				
										<TR>
											<TD vAlign="top" height="100%" width=100%>
												<table cellSpacing="0" cellPadding="0" height="100%" width=100%>
													<tr>
														<td>
															<uc1:right runat="server" id="RightHeader"></uc1:right>
														</td>
													</tr>
												</table>
											</TD>
											<TD vAlign="top" width="100%" height="100%">
												<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0">
													<tr>
														<td vAlign="top" align=left>
															<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0">
																<TR>
																	<TD align=left bgcolor="#3366cc">
																		
																		<uc4:right runat="server" id="headerbutton"></uc4:right>
																	</TD>
																</TR>
															</TABLE>
														</td>
													</tr>
													<TR><td height=1></td></tr>
													<tr>
														<td align="left" colSpan="4">
															
												<TABLE cellSpacing="0" cellPadding="0" width="100%" border="0">
													<TR>
														<td Width="100%" class="INPUTTITLETEXT" valign=middle Height="25">
																		<!-- Content goes here -->
																		 <asp:Label runat=server ID="lblBTC" CssClass="head00_8_5"></asp:Label>
																		
																	
														                </td>
													</TR>
													<TR>
														<td height="15">
														</td>
													</TR>
													<TR>
														<td Height="115" valign="top">
															<TABLE width="100%" height="130"  BORDER="0" CELLSPACING="0" CELLPADDING="0">
																<TR>
																
																<td width=6 valign=top></td>
																
																	<TD valign="top" align=center>
																	
																	  <table >
																	  
																	  
																	   <tr>
																	          <td class="head05">
        																	          Finalize payment with OnParticipe
															                  </TD>
																	      </tr>
																	      
																	      <tr>
																	          <td class="head06">
        																	         
															                  </TD>
																	      </tr>
																	      
																	      
																	       <tr>
																	          <td height=70>
        																	         
															                  </TD>
																	      </tr>
																	      
																	        <tr>
																	          <td class=BlueText align=center>
        																	        To complete the payment please click button [Finish payment] and get redirected to our payment gateway partner
															                        <br />
                                                                                    OnParticipe is the best payments collecting company in Europe</TD>
																	      </tr>
																	      
																	       <tr>
																	          <td height=10>
        																	         
															                  </TD>
																	      </tr>
																	      
																	       <tr>
																	          <td class=BlueText align=center >
        																	       <asp:button runat=server id="btnSubmit" text="Finish payment" cssclass=INPUTBUTTON width=200 height=40 />
															                  </TD>
																	      </tr>
																	      
																	       <tr>
																	          <td height=70>
        																	         
															                  </TD>
																	      </tr>
																	      
																	        <tr>
																	          <td height=10 align=center class=BlackSmallText10>
        																	        1 - In first field write your Rpchost UserID 
															                        , in second field the amount you want to deposit with Rpchost.com</TD>
																	      </tr>
																	      
																	       <tr>
																	          <td align=center class=head01_9>
        																	      
        																	      <asp:image runat=server ID=img1 Imageurl="picture\Participe1.jpg" width=50%></asp:image>
        																	       
        																	  </TD>
																	      </tr>
																	      
																	       <tr>
																	          <td height=10 class=head06_small align=center>
        																	         &nbsp;</TD>
																	      </tr>
																	      
																	      
																	      
																	      
																	      
																	      
																	      
																	        <tr>
																	          <td height=10 class=BlackSmallText10 align=center> 
        																											     
        																	       
															                      2 - Enter your credit card information</TD>
																	      </tr>
																	      
																	        <tr>
																	          <td align=center class="head06_small" valign=top> 
        																	        <asp:image runat=server ID=img2 Imageurl="picture\Participe2.jpg" width=50%></asp:image>
        																	     </TD>
																	      </tr>
																	      
																	      <tr>
																	          <td align=center class=BlueSmallText > 
        																	       
        																	       Powered by OnParticipe
        																	       </TD>
																	      </tr>
																	      
																	     </table>
																		
																		
																	</TD>
																	
																	<td valign=top>
																	   <%-- <img src="picture/Lending.jpg"--%>
																	</td>
														
																</TR>
															</TABLE>
														</td>
													</TR>
												</TABLE>
											</td>
										</tr>
										
										
													
													
												</TABLE>
											</td>
										</tr>
									</TABLE>
										</td>
									</tr>
								</table>
							</TD>
						</TR>
					</TABLE>
				</TD>
			</TR>
		</TABLE>
	
	
	
</TD>
</TR>




		<!-- Start of eWebCounter Code -->
		<script src="http://www.google-analytics.com/urchin.js" type=text/javascript></script>
		<script type="text/javascript" src="eWebCounter.js" ></script>
		<!-- End of eWebCounter Code -->
</form>

	</body>
</HTML>
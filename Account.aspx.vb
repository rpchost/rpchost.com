...
Partial Class Account
    ...
    Protected Sub btnConfirmWithdrawUSD_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnConfirmWithdrawUSD.Click
        ...
        Else
            ...
            Dim opExec As New rpchost.cs_Operation
            opExec.ExecuteSql("insert into WithdrawalRequests (UserID,WithdrawUsd,DateRequest,WithdrawMethod) values (" & Session("UserID") & "," & Request("txtUsdTobeWithdrawn") & ",'" & Logger.GenerateTimeZone("Eastern Standard Time") & "','" & RdWithdraw.SelectedValue & "') ")
            opExec.closeConnection()
    ...
    Protected Sub btnConfirmWithdrawBTC_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnConfirmWithdrawBTC.Click
        ...
        Dim opExec As New rpchost.cs_Operation
        opExec.ExecuteSql("insert into WithdrawalRequests (UserID,WithdrawBtc,DateRequest) values (" & Session("UserID") & "," & txtBTCWithdraw.Text.Trim & ",'" & Logger.GenerateTimeZone("Eastern Standard Time") & "') ")
        opExec.closeConnection()
    ...
    Protected Sub btnExchangeUSD_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExchangeUSD.Click
        ...
        Dim opExec As New rpchost.cs_Operation
        opExec.ExecuteSql("insert into ExchangeRequest (UserID,UsdAmountSentFromAccount,DateRequest) values (" & Session("UserID") & "," & Request("txtExchangeUSD") & ",'" & Logger.GenerateTimeZone("Eastern Standard Time") & "') ")
        opExec.closeConnection()
    ...
    Protected Sub btnExchangeBTC_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExchangeBTC.Click
        ...
        Dim opExec As New rpchost.cs_Operation
        opExec.ExecuteSql("insert into ExchangeRequest (UserID,BtcAmountSentFromAccount,DateRequest) values (" & Session("UserID") & "," & Request("txtExchangeBTC") & ",'" & Logger.GenerateTimeZone("Eastern Standard Time") & "') ")
        opExec.closeConnection()
    ...
    Protected Sub btnTransferUSD_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnTransferUSD.Click

        If (Request("txtUsdUserID").Trim <> "") Then

            If (isToUserValid(Request("txtUsdUserID").Trim) = False) Then
                Response.Write("<script>alert('The UserID you provided is not valid Rpchost user')</script>")
                txtUsdUserID.Text = ""
                Return
            End If
        ...
        Else
            ...
            Response.Write("<script>alert('The UserID you provided must not be empty')</script>")
            txtUsdUserID.Text = ""
            Return
        ...
        Try
            ...
            If (Convert.ToDouble(Request("txtTransferUSD").Trim) <= 0) Then

                Response.Write("<script>alert('USD Transfer Amount must be positive number')</script>")

                Return
            End If
            If (Request("txtTransferUSD").Trim.Equals("")) Then
                Response.Write("<script>alert('USD Transfer Amount must not be empty')</script>")
                Return
            End If

        Catch ex As Exception
            Response.Write("<script>alert('USD Transfer Amount must be valid number')</script>")
            Return
        End Try
        ...
        Try
            ...
            If (Convert.ToDouble(Request("txtUsdUserID").Trim) <= 0) Then

                Response.Write("<script>alert('UTo UserID field must be positive number')</script>")

                Return
            End If
            If (Request("txtUsdUserID").Trim.Equals("")) Then
                Response.Write("<script>alert('To UserID field must not be empty')</script>")
                Return
            End If

        Catch ex As Exception
            Response.Write("<script>alert('To UserID must be valid number')</script>")
            Return
        End Try


        If (Convert.ToDouble(txtTransferUSD.Text.Trim) > Convert.ToDouble(txtUsdBalance.Text.Trim)) Then

            Response.Write("<script>alert('Transfer Amount should be less then the USD Main Balance')</script>")
        ...
        Dim opExec As New rpchost.cs_Operation
        opExec.ExecuteSql("insert into Transfer (UserID,ToUserID,UsdAmount,BtcAmount,DateTransfer) values (" & Session("UserID") & "," & txtUsdUserID.Text & "," & txtTransferUSD.Text & ",0,'" & Logger.GenerateTimeZone("Eastern Standard Time") & "') ")
        opExec.closeConnection()

        txtUsdUserID.Text = ""
        txtTransferUSD.Text = "0"

        Response.Redirect("Account.aspx")

    End Sub

    Protected Sub btnTransferBtc_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnTransferBtc.Click

        If (Request("txtBtcUserID").Trim <> "") Then

            If (isToUserValid(Request("txtBtcUserID").Trim) = False) Then
                Response.Write("<script>alert('The UserID you provided is not valid Rpchost user')</script>")
                txtUsdUserID.Text = ""
                Return
            End If
        ...
        Try
            ...
            If (Convert.ToDouble(Request("txtTransferBTC").Trim) <= 0) Then

                Response.Write("<script>alert('BTC Transfer Amount must be positive number')</script>")

                Return
            End If
            If (Request("txtTransferBTC").Trim.Equals("")) Then
                Response.Write("<script>alert('BTC Transfer Amount must not be empty')</script>")
                Return
            End If

        Catch ex As Exception
            Response.Write("<script>alert('BTC Transfer Amount must be valid number')</script>")
            Return
        End Try
        ...
        Try
            ...
            If (Convert.ToDouble(Request("txtBtcUserID").Trim) <= 0) Then

                Response.Write("<script>alert('To UserID field must be positive number')</script>")

                Return
            End If
            If (Request("txtBtcUserID").Trim.Equals("")) Then
                Response.Write("<script>alert('To UserID field must not be empty')</script>")
                Return
            End If
        ...
        If (Convert.ToDouble(txtTransferBtc.Text.Trim) > Convert.ToDouble(txtBtcBalance.Text.Trim)) Then

            Response.Write("<script>alert('Transfer Amount should be less then the BTC Main Balance')</script>")
        ...
        Dim opExec As New rpchost.cs_Operation
        opExec.ExecuteSql("insert into Transfer (UserID,ToUserID,UsdAmount,BtcAmount,DateTransfer) values (" & Session("UserID") & "," & txtBtcUserID.Text & ",0," & txtTransferBtc.Text & ",'" & Logger.GenerateTimeZone("Eastern Standard Time") & "') ")
        opExec.closeConnection()

        txtBtcUserID.Text = ""
        txtTransferBtc.Text = "0"
